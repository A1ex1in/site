...
router.get("/groups/:id/students",requireAuth,requireTeacher,async (request, response) => {
        ...
        try {
            ...
            console.error(
                "Ошибка получения студентов группы:",
                error
            );

            response.status(500).json({
                error: "Ошибка получения студентов группы"
            });
        }
    }
);
...
router.get("/students/:id",requireAuth,requireTeacher,async (request, response) => {
        ...
        try {
            ...
            if (!/^\d+$/.test(studentId)) {
                return response.status(400).json({
                    error: "Некорректный идентификатор студента"
                });
            }
...
router.patch("/students/:id/group",requireAuth,requireTeacher,async (request, response) => {
        ...
        try {
            ...
            if (!groupId || !/^\d+$/.test(String(groupId))) {
                return response.status(400).json({
                    error: "Необходимо указать корректную группу"
                });
            }
            ...
            if (groupResult.rowCount === 0) {
                return response.status(404).json({
                    error: "Учебная группа не найдена"
                });
            }
            ...
            response.json({
                message: "Группа студента изменена"
            });

        } catch (error) {

            console.error(
                "Ошибка изменения группы студента:",
                error
            );

            response.status(500).json({
                error: "Ошибка изменения группы студента"
            });
        }
    }
);
...
router.patch("/students/:id/status",requireAuth,requireTeacher,async (request, response) => {
        ...
        try {
            ...
            if (!allowedStatuses.includes(status)) {
                return response.status(400).json({
                    error: "Недопустимый статус студента"
                });
            }
            ...
            console.error(
                "Ошибка изменения статуса студента:",
                error
            );

            response.status(500).json({
                error: "Ошибка изменения статуса студента"
            });
        }
    }
);
